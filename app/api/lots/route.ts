...
function calcStatus(expirationDate: Date): LotStatus {
  ...
  const diffDays = Math.ceil((d1.getTime() - d0.getTime()) / (1000 * 60 * 60 * 24))
  if (diffDays <= 0) return 'EXPIRED'
  if (diffDays <= 14) return 'NEAR_EXPIRE'
  return 'USABLE'
}

// GET /api/lots?search=&status=&vaccineId=&from=&to=&page=1&limit=20
export async function GET(req: Request) {
  const q = new URL(req.url).searchParams
  const search = q.get('search') ?? ''
  const status = q.get('status') ?? '' // USABLE | NEAR_EXPIRE | EXPIRED
  ...
  const limit = Math.min(100, Math.max(1, parseInt(q.get('limit') ?? '20')))

  // หมายเหตุ: MySQL ส่วนใหญ่ collation จะ case-insensitive อยู่แล้ว
  // ถ้าของคุณเป็น case-sensitive ให้จัดการที่ collation ของคอลัมน์แทน
  const where: any = {
    AND: [
      search
        ? {
            OR: [
              ...
              // ค้นหาข้าม relation ต้องห่อด้วย is:{}
  ...
  const [items, total] = await Promise.all([
    prisma.vaccineLot.findMany({
      ...
      // ให้รายการที่เพิ่งสร้างติดหน้าแรกเสมอ
      orderBy: [{ createdAt: 'desc' }, { expirationDate: 'asc' }, { updatedAt: 'desc' }],
      skip: (page - 1) * limit,
      take: limit,
    }),
...
export async function POST(req: Request) {
  ...
  try {
    ...
    return NextResponse.json({ message: 'invalid json' }, { status: 400, headers: { 'Cache-Control': 'no-store' } })
  }
  ...
  if (dup) {
    return NextResponse.json({ message: 'มีรหัสล็อตนี้อยู่แล้ว' }, { status: 409, headers: { 'Cache-Control': 'no-store' } })
  }

  const v = await prisma.vaccine.findUnique({ where: { id: Number(vaccineId) } })
  if (!v) {
    return NextResponse.json({ message: 'ไม่พบ Vaccine ตามที่ระบุ' }, { status: 404, headers: { 'Cache-Control': 'no-store' } })
  }

  const expDate = parseDateOnly(String(expirationDate))
  if (isNaN(expDate.getTime())) {
    return NextResponse.json({ message: 'รูปแบบวันหมดอายุไม่ถูกต้อง' }, { status: 400, headers: { 'Cache-Control': 'no-store' } })
  }
